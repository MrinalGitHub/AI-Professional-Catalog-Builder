/*
 * DESIGN: Lookbook Catalogue Style — Shared Component
...
interface BatteryOption {
  ...
  range: string;
  price: number; // Store as number for markup calculation
  charging: string;
  charger: string;
  warranty: string;
}
...
export type CatalogueMode = "dealer" | "customer";

// ─── DATA (Dealer Prices — Source of Truth) ─────────────────────────────────
...
const models: ModelData[] = [
  {
    ...
    batteries: [
      { config: "Without Battery", type: "base", range: "—", price: 20000, charging: "—", charger: "—", warranty: "—" },
      { config: "Lead Acid 48V", type: "lead", range: "50 km", price: 31000, charging: "5–6 Hours", charger: "4AMP Auto cut", warranty: "1 Year" },
      { config: "Lead Acid 60V", type: "lead", range: "70 km", price: 33500, charging: "5–6 Hours", charger: "4AMP Auto cut", warranty: "1 Year" },
      { config: "Lithium 48V / 32Ah", type: "lithium", range: "50 km", price: 34500, charging: "3–4 Hours", charger: "6AMP Auto cut", warranty: "2 Years" },
      { config: "Lithium 60V / 36Ah", type: "lithium", range: "70 km", price: 40000, charging: "3–4 Hours", charger: "6AMP Auto cut", warranty: "2 Years" },
    ],
  },
  {
    ...
    batteries: [
      { config: "Without Battery", type: "base", range: "—", price: 25000, charging: "—", charger: "—", warranty: "—" },
      { config: "Lithium 60V / 36Ah", type: "lithium", range: "70 km", price: 45000, charging: "3–4 Hours", charger: "6AMP Auto cut", warranty: "2 Years" },
      { config: "Lithium 60V / 45Ah", type: "lithium", range: "80 km", price: 51000, charging: "3–4 Hours", charger: "6AMP Auto cut", warranty: "2 Years" },
    ],
  },
  {
    ...
    batteries: [
      { config: "Without Battery", type: "base", range: "—", price: 32000, charging: "—", charger: "—", warranty: "—" },
      { config: "Lithium 60V / 36Ah", type: "lithium", range: "70 km", price: 53000, charging: "3–4 Hours", charger: "6AMP Auto cut", warranty: "2 Years" },
      { config: "Lithium 60V / 45Ah", type: "lithium", range: "80 km", price: 58000, charging: "3–4 Hours", charger: "6AMP Auto cut", warranty: "2 Years" },
      { config: "Lithium 60V / 100Ah", type: "lithium", range: "130 km", price: 79999, charging: "3–4 Hours", charger: "6AMP Auto cut", warranty: "2 Years" },
    ],
  },
];
...
const MARKUP = 1.32; // 32% markup for customer pricing

function formatPrice(price: number, mode: CatalogueMode): string {
  const finalPrice = mode === "customer" ? Math.round(price * MARKUP) : price;
  return `₹${finalPrice.toLocaleString("en-IN")}`;
}

// ─── BORDER & WATERMARK STYLES ──────────────────────────────────────────────

function getBorderClass(mode: CatalogueMode): string {
  return mode === "dealer"
    ? "border-[6px] border-green-600"
    : "border-[6px] border-orange-500";
}

function getWatermarkClass(mode: CatalogueMode): string {
  return mode === "dealer"
    ? "text-green-200"
    : "text-orange-200";
}

function getWatermarkText(mode: CatalogueMode): string {
  return mode === "dealer" ? "DEALER COPY" : "CUSTOMER COPY";
}
...
function ModelPage({ model, index, mode }: { model: ModelData; index: number; mode: CatalogueMode }) {
  return (
    <section
              ...
              <tbody>
                {model.batteries.map((bat) => (
                  <tr key={bat.config} className="border-b border-gray-100 hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="py-3 pr-4">
                      <span className="font-bold text-gray-900 text-base">{formatPrice(bat.price, mode)}</span>
                    </td>
...
function Footer({ mode }: { mode: CatalogueMode }) {
  return (
    <footer className={`bg-gray-50 border-t border-gray-200 py-6 ${getBorderClass(mode)}`}>
...
function StickyNav({ mode }: { mode: CatalogueMode }) {
  const otherMode = mode === "dealer" ? "customer" : "dealer";
  const otherLabel = mode === "dealer" ? "Customer Pricing" : "Dealer Pricing";
  const otherPath = mode === "dealer" ? "/customer" : "/dealer";

  return (
    <nav className={`sticky top-0 z-50 bg-white/95 backdrop-blur-sm border-b border-gray-100 ${
      mode === "dealer" ? "border-l-[6px] border-r-[6px] border-l-green-600 border-r-green-600" : "border-l-[6px] border-r-[6px] border-l-orange-500 border-r-orange-500"
    }`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between py-2.5">
        ...
        <a href={mode === "dealer" ? "/dealer" : "/customer"} className="flex-shrink-0">
        ...
        {/* Switch to other pricing */}
        <a
          href={otherPath}
          className={`hidden lg:inline-block text-xs font-bold px-3 py-1.5 rounded-full transition-colors ${
            mode === "dealer"
...
export default function CataloguePage({ mode }: { mode: CatalogueMode }) {
  return (
    <div className="min-h-screen bg-white">
      <CoverSection mode={mode} />
      <StickyNav mode={mode} />
      {models.map((model, i) => (
        <ModelPage key={model.name} model={model} index={i} mode={mode} />
      ))}
